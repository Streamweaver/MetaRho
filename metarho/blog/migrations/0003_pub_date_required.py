...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        "Take all posts with blank pub_dates and assign the date created as pub_date"
        posts = Post.objects.filter(pub_date__isnull=True)
        for post in posts:
            post.pud_date = post.date_created
            post.save()


    def backwards(self, orm):
        "Remove pub_date from unpublished posts"
        posts = Post.objects.filter(status=UNPUBLISHED_STATUS)
        for post in posts:
            post.pub_date = None
            post.save()
